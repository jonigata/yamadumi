...
public:
    ...
    substr get() {
        ...
        throw mqo_reader_error(
            "bad token: " + buffer_.substr(prev_, curr_ - prev_ ));
    }
...
public:
    ...
    void expect_literal(const char* s) {
        ...
        if (get( token ) != s){
            throw mqo_reader_error(
                "unexpected token '" + token.str() + "' for '" + s + "'" );
        }
    }
    ...
    substr expect_string() {
        ...
        if(token[0] != '"' || token[token.length()-1] != '"') {
            throw mqo_reader_error(
                "unexpected token '" + token.str() + "' for string" );
        }
        return substr(
            token.buffer(),
            token.index() + 1,
    ...
    substr expect_string(size_t size) {
        ...
        if (size < token.length()) {
            throw mqo_reader_error("too long string " + token.str());
        }
        return token;
    }
    ...
    int expect_integer() {
        substr token = get();
        try {
            int n = std::stoi(token.str());
            return n;
        }
        catch(...){
            throw mqo_reader_error(
                "unexpected token '" + token.str() + "for integer" );
        }
    }
    ...
    uint32_t expect_dword() {
        substr token = get();
        char* p;
        uint32_t x = strtoul(token.str().c_str(), &p, 10);
        if (!p) {
            throw mqo_reader_error(
    ...
    float expect_float() {
        substr token = get();
        try {
            float n = std::stof(token.str());
            return n;
        }
        catch(...) {
            throw mqo_reader_error(
                "unexpected token '" + token.str() + "for float" );
        }
    }
